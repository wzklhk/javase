package com.example.leetcode;

import org.junit.Test;

/**
 * @author wzklhk
 * <p>
 * 给你一个 n x n 整数矩阵 grid ，请你返回 非零偏移下降路径 数字和的最小值。
 * <p>
 * 非零偏移下降路径 定义为：从 grid 数组中的每一行选择一个数字，且按顺序选出来的数字中，相邻数字不在原数组的同一列。
 * <p>
 * <p>
 * <p>
 * 示例 1：
 * <p>
 * <p>
 * <p>
 * <p>
 * 输入：grid = [[1,2,3],[4,5,6],[7,8,9]]
 * 输出：13
 * 解释：
 * 所有非零偏移下降路径包括：
 * [1,5,9], [1,5,7], [1,6,7], [1,6,8],
 * [2,4,8], [2,4,9], [2,6,7], [2,6,8],
 * [3,4,8], [3,4,9], [3,5,7], [3,5,9]
 * 下降路径中数字和最小的是 [1,5,7] ，所以答案是 13 。
 * <p>
 * <p>
 * 示例 2：
 * <p>
 * <p>
 * 输入：grid = [[7]]
 * 输出：7
 * <p>
 * <p>
 * <p>
 * <p>
 * 提示：
 * <p>
 * <p>
 * n == grid.length == grid[i].length
 * 1 <= n <= 200
 * -99 <= grid[i][j] <= 99
 * <p>
 * <p>
 * Related Topics 数组 动态规划 矩阵 👍 174 👎 0
 */


public class Problem1289HardMinFallingPathSumTest {
    public int minFallingPathSumDP(int[][] grid) {
        int n = grid.length;
        // 动态规划，存储该点之上的最小和
        int[][] dp = new int[n][n];

        // init
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                dp[i][j] = Integer.MAX_VALUE;
            }
        }
        for (int i = 0; i < n; i++) {
            dp[0][i] = grid[0][i];
        }


        for (int i = 1; i < n; i++) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < n; k++) {
                    if (j == k) {
                        continue;
                    }
                    dp[i][j] = Math.min(dp[i][j], dp[i - 1][k] + grid[i][j]);
                }
            }
        }

        // 找到DP矩阵最小的解
        int res = Integer.MAX_VALUE;
        for (int j = 0; j < n; j++) {
            res = Math.min(res, dp[n - 1][j]);
        }
        return res;
    }

    @Deprecated
    public int minFallingPathSumGreedUnsolved(int[][] grid) {
        int n = grid.length;
        int sum = 0;
        int prevMinIndex = -1;
        for (int i = 0; i < n; i++) {
            int min = Integer.MAX_VALUE;
            int minIndex = 0;
            for (int j = 0; j < n; j++) {
                if (j != prevMinIndex && grid[i][j] < min) {
                    minIndex = j;
                    min = grid[i][j];
                }
            }
            sum += min;
            prevMinIndex = minIndex;
        }
        return sum;
    }

    public int minFallingPathSumDP2(int[][] grid) {
        int n = grid.length;

        int firstMinSum = 0;
        int secondMinSum = 0;
        int firstMinIndex = -1;

        for (int i = 0; i < n; i++) {
            int curFirstMinSum = Integer.MAX_VALUE;
            int curSecondMinSum = Integer.MAX_VALUE;
            int curFirstMinIndex = -1;

            for (int j = 0; j < n; j++) {
                int curSum = (j != firstMinIndex ? firstMinSum : secondMinSum) + grid[i][j];
                if (curSum < curFirstMinSum) {
                    curSecondMinSum = curFirstMinSum;
                    curFirstMinSum = curSum;
                    curFirstMinIndex = j;
                } else if (curSum < curSecondMinSum) {
                    curSecondMinSum = curSum;
                }
            }
            firstMinSum = curFirstMinSum;
            secondMinSum = curSecondMinSum;
            firstMinIndex = curFirstMinIndex;
        }
        return firstMinSum;
    }

    @Test
    public void test() {
        int[][] grid = {{1, 2, 3}, {4, 5, 6}, {7, 8, 9}};
        int x = minFallingPathSumDP2(grid);
        System.out.println(x);
    }

    @Test
    public void test2() {
        int[][] grid = {{2, 2, 1, 2, 2}, {2, 2, 1, 2, 2}, {2, 2, 1, 2, 2}, {2, 2, 1, 2, 2}, {2, 2, 1, 2, 2}};
        int x = minFallingPathSumDP2(grid);
        System.out.println(x);
    }
}
